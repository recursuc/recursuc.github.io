...
draw2d.CommandAttribute = function(figure, attrName, newVal, oldVal) {
	draw2d.Command.call(this, "modify attribute");
	this.figure = figure;
	this.attrName = $O.type(attrName) == "array" ? attrName : [attrName];
	this.newVal = $O.type(newVal) == "array" ? newVal : [newVal];
	this.oldVal = $O.type(oldVal) == "array" ? oldVal : [oldVal];
};
draw2d.CommandAttribute.prototype = new draw2d.Command();
draw2d.CommandAttribute.prototype.type = "draw2d.CommandAttribute";
draw2d.CommandAttribute.prototype.execute = function() {
	this.redo();
};
draw2d.CommandAttribute.prototype.redo = function() {
	this.setValue("newVal");
};
draw2d.CommandAttribute.prototype.undo = function() {
	this.setValue("oldVal");
};
draw2d.CommandAttribute.prototype.setValue = function(type) {
	var curObj = this.figure,
		extAttrs = curObj.ExtAttributes,
		aNames = this.attrName,
		aVals = this[type];
	for(var i = aNames.length - 1; i >= 0; i--){
		extAttrs[aNames[i]] = aVals[i];
	}
	"Name" == aNames[0] && curObj.setTitle && curObj.setTitle(aVals[0]);
};
